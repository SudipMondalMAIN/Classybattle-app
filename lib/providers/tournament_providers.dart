...
final selectedTournamentTabProvider =
    StateProvider<TournamentTab>((ref) => TournamentTab.all);
...
final allTournamentsProvider = FutureProvider<List<TournamentModel>>((ref) async {
...
final tournamentsForSelectedTabProvider =
  ...
  if (tab == TournamentTab.mine) {
    try {
      ...
      final ids = regs.items.where((r) => r.isActive).map((r) => r.tournamentId).toSet();
      if (ids.isEmpty) return [];
      final all = await tournamentService.fetchTournaments(pageSize: 100);
      var mine = all.items.where((t) => ids.contains(t.id)).toList();
      if (gameId != null) mine = mine.where((t) => t.gameId == gameId).toList();
...
final myTournamentStatsProvider = FutureProvider<MyTournamentStats?>((ref) async {
  try {
...
final tournamentMapProvider =
    FutureProvider.family<MapModel?, String?>((ref, mapId) {
...
final myRegistrationProvider =
    FutureProvider.family<ParticipantModel?, String>((ref, tournamentId) {
